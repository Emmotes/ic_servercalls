...
function togglePullButtons(disable) {
	...
	for (let name of pbNames) {
		...
		if (disable) {
			...
			createTimer(globalButtonDisableTime,`mpb_${name}`,`${name}PullButtonDisabled`,`<span id="${name}PullButtonDisabled" style="font-size:0.9em" hidden>Still busy with other tasks. Please wait.</span>`,prefix,suffix);
		}
