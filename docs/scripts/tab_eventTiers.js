const vet = 1.200; // prettier-ignore
...
function et_changeOrder(type) {
	if (type == null) {
		...
		type = sortEle?.value || et_DEFAULT_eventSort;
	}
	et_setEventSort(type);
	const wrapper = document.getElementById("eventTiersWrapper");
...
function et_buildSVG(tier, month, day) {
	if ((month === 10 && day >= 24) || (month === 11 && day === 1))
		return et_svgHalloween(tier);
	if (month === 2 && day >= 7 && day <= 15) return et_svgValentines(tier);
	if (month === 8 && day >= 1 && day <= 9) return et_svgCats(tier);
	if (month === 7 && day >= 15 && day <= 23) return et_svgPiDay(tier);

	const flags = f_getSiteFlags();
	if (flags.has(`halloween`)) return et_svgHalloween(tier);
	if (flags.has(`valentines`)) return et_svgValentines(tier);
	if (flags.has(`cats`)) return et_svgCats(tier);
	if (flags.has(`piday`)) return et_svgPiDay(tier);
...
function et_svgValentines(tier) {
...
function et_svgCats(tier) {
	return `<svg height="30" width="30" viewBox="-25 -25 562 562" xmlns="http://www.w3.org/2000/svg" stroke="#000" stroke-width="12"><g stroke-linecap="round" stroke-linejoin="round"/><path class="svgEventTier${tier}" style="filter:drop-shadow(0 0 20px)" d="M504.302 195.59c-10.618-6.362-29.71-43.514-33.959-47.755-4.249-4.256-28.662-13.81-28.662-13.81l-13.795-43.514c-26.542 3.185-42.449 45.634-43.513 48.819 0 0-2.661 73.764-120.47 45.11-84.437-20.54-135.22-40.407-177.872-.36-2.668-4.578-4.491-9.234-5.822-14.257-2.378-9.062-2.918-19.46-2.91-31.308 0-5.806.109-11.924.117-18.34-.008-13.436-.493-28.248-4.046-44.015-3.52-15.774-10.234-32.417-22.026-48.998-5.086-7.168-15.024-8.858-22.199-3.764-7.167 5.094-8.858 15.031-3.764 22.199 9.304 13.145 14.162 25.368 16.932 37.551 2.739 12.167 3.271 24.397 3.264 37.027 0 6.024-.118 12.136-.118 18.34.016 12.646.438 25.799 3.928 39.32 2.746 10.735 7.722 21.58 15.508 31.706-32.965 51.917-14.858 107.043-14.858 107.043l-34.093 54.241a9.1 9.1 0 0 0-1.244 3.192L.146 454.073a9.02 9.02 0 0 0 3.819 9.084l40.979 27.763a3.57 3.57 0 0 0 2.981.493 3.57 3.57 0 0 0 2.308-1.949l8.326-17.825a3.64 3.64 0 0 0-.634-3.998l-10.728-11.448a9.03 9.03 0 0 1-2.081-8.701l17.84-61.033a9.08 9.08 0 0 1 3.435-4.82l23.983-17.058a9.004 9.004 0 0 1 10.806.251 9.03 9.03 0 0 1 2.809 10.438L93.41 401.858a9.01 9.01 0 0 0 1.401 9.06l60.298 73.365h45.218a3.615 3.615 0 0 0 3.616-3.607v-22.504a3.617 3.617 0 0 0-3.616-3.615h-14.303a9.06 9.06 0 0 1-7.982-4.804l-24.484-41.049a9.02 9.02 0 0 1 .783-9.663l21.948-28.999a9.04 9.04 0 0 1 7.192-3.575h61.197a9.02 9.02 0 0 1 7.277 3.701 9.06 9.06 0 0 1 1.33 8.059l-19.726 62.16a9 9 0 0 0 .18 5.994l12.653 32.598h39.272v-8.49l-6.494-20.415a9 9 0 0 1 .133-5.852l26.471-71.847a9.04 9.04 0 0 1 8.474-5.908h27.011a9.02 9.02 0 0 1 6.502 2.777l74.813 103.365h50.947c12.738 0 7.809-25.414-4.241-26.542-22.826-2.12-52.004-65.798-64.742-88.09-12.739-22.285 24.664-91.299 53.067-94.453 28.654-3.185 43.514-14.867 47.763-30.79 8.488-9.547 9.552-30.775-1.066-37.144z"/></svg>`;
}

function et_svgPiDay(tier) {
	return `<svg width="30" height="30" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" stroke="#000" stroke-width=".5"><circle class="svgEventTier${tier}" style="filter:drop-shadow(0 0 1.5px)" cx="10" cy="10" r="8"/></svg>`;
}

function et_initEventTiersSettings() {
	let hideTier4Ele = document.getElementById(`eventTiersHideTier4`);
	if (hideTier4Ele) hideTier4Ele.checked = et_getHideTier4();
	let sortTypeEle = document.getElementById(`eventTiersSort`);
	if (sortTypeEle) sortTypeEle.value = et_getEventSort();
}
...
function et_getHideTier4() {
	return ls_getGlobal(et_LSKEY_hideTier4, false) === 1;
}

function et_setHideTier4(hide) {
	ls_setGlobal_bool(et_LSKEY_hideTier4, hide, false);
}

function et_getEventSort() {
	return ls_getGlobal(et_LSKEY_eventSort, et_DEFAULT_eventSort);
}

function et_setEventSort(eventSort) {
	return ls_setGlobal(
		et_LSKEY_eventSort,
		eventSort,
		(v) => v == null || v === `` || v === et_DEFAULT_eventSort,
	);
}
