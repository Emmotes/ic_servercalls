const vet = 1.014; // prettier-ignore
...
async function et_displayEventTiersData(
	...
	for (let hero of heroDefs) {
		...
		if (
			!collections.hasOwnProperty(heroId) ||
			!collections[heroId].hasOwnProperty("v")
		)
			continue;
		let eventName = hero.event_name;
		let event2Id = et_getEvent2IdFromEventName(eventName);
		if (event2Id != undefined) eventNameByEventId[event2Id] = eventName;
...
function et_parseOwnedChamps(defsHeroes, detailsHeroes) {
	...
	for (let hero of defsHeroes) {
		...
		if (ownedIds.includes(hero.id)) {
			...
			if (event2Id != undefined) ownedEventById[hero.id] = event2Id;
		}
	}
...
function et_changeOrder(type) {
	...
	for (let ele of wrapper.childNodes) {
		...
		else ele.style.order = eventId * eventIdMult + nameId;
	}
}
...
function et_getCurrentOrNextEventId(details) {
	if (
		details.hasOwnProperty("active_events") &&
		details.active_events.length == 1
	)
		return details.active_events[0].event_id;
	if (
		details.hasOwnProperty("next_event") &&
		details.next_event.hasOwnProperty("event_id")
	)
...
function et_getHideTier4() {
	...
	if (strg != undefined) return strg != "0";
	return false;
}
