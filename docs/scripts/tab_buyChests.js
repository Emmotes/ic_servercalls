const vbc = 1.018; // prettier-ignore
...
async function bc_pullBuyChestsData() {
	...
	try {
		...
		let chests = (await getDefinitions("chest_type_defines"))
			.chest_type_defines;
		...
		outerLoop: for (let chestId of eventChestIds) {
			for (let chest of chests) {
				if (chest.id == chestId) {
					...
					dismantle = chest.hero_ids.some((r) =>
...
function bc_modifyBuyChestsBuyAmountSlider(val) {
	...
	let be = ``;
	let maximumToUse;
	if (val == undefined) maximumToUse = 0;
	else if (val == 1) maximumToUse = Number(buyChestsSilver.value);
	else if (val == 2) maximumToUse = Number(buyChestsGold.value);
	else maximumToUse = Number(buyChestsEvent.value);
...
async function bc_buyChests() {
	...
	let buyChestsCurrencyDisplay = document.getElementById(
		chestId > 2 ? `buyChestsTokensDisplay` : `buyChestsGemsDisplay`
	);
	let buyChestsMaxType = document.getElementById(
		chestId == 1 ? `buyChestsTokensDisplay` : `buyChestsGemsDisplay`
	);
	...
	while (amount > 0 && numFails < RETRIES) {
		...
		if (result["success"] && result["okay"]) {
			...
			if (chestId > 2) {
				...
				bc_applyValueToElementAndDisplay(
					`buyChestsEvent`,
					Math.floor(currencyRemaining / chestPackCost)
				);
			} else {
				bc_applyValueToElementAndDisplay(
					`buyChestsGems`,
				...
				bc_applyValueToElementAndDisplay(
					`buyChestsSilver`,
					Math.floor(currencyRemaining / silverChestCost)
				);
				bc_applyValueToElementAndDisplay(
					`buyChestsGold`,
					Math.floor(currencyRemaining / goldChestCost)
				);
			}
