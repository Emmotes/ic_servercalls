const vss = 1.006; // prettier-ignore
...
async function ss_displayServerStatusData(wrapper) {
	...
	for (let serverToTest of serversToTest) {
		...
		try {
			...
			const response = await getPlayServerForDefinitions(url, 5000);
			if (response.success && response["play_server"] != null) {
				const elapsed = Date.now() - start;
				responses[serverToTest] = {alive: true, time: elapsed};
				const told = response["play_server"]
					.replace(/https?:\/\//gi, "")
					.replace(/\.idlechampions.*/gi, "");
				if (!serversToTest.includes(told)) serversToTest.push(told);
			}
		} catch (error) {
	...
	for (let serverToTest of serversToTest) {
		const alive = responses[serverToTest].alive
			? ss_SVG_tick
			: ss_SVG_cross;
		let response = ss_displayTime(responses[serverToTest].time);
		if (responses[serverToTest].time >= 5000) response += ` (Timed Out)`;
