const vtm = 1.010; // prettier-ignore
var roles;
var champsById;
var champsByName;
var diffs;
...
async function tm_displayData(
	...
	if (
		trialsData != undefined &&
		trialsData.highest_available_difficulty != undefined &&
		...
		wrapper.innerHTML = `&nbsp;`;
		if (
			userDetails.trials_system_unlocked == undefined ||
	...
	if (trialsData.pending_unclaimed_campaign != undefined) {
		...
		(trialsData != undefined && trialsData.active_campaign_id == 0) ||
		campaigns == undefined ||
		campaigns.length != 1 ||
...
async function tm_createTrial() {
	let trialsInfo = document.getElementById(`trialsInfo`);
	let trialsDifficultySelect = document.getElementById(
		`trialsDifficultySelect`
	);
	let trialsPrivateCheckbox = document.getElementById(
		`trialsPrivateCheckbox`
	);
	let trialsAutostartCheckbox = document.getElementById(
	...
	let response = await trialsCreateCampaign(
		diffId,
		isPrivate,
		0,
		isAutostart
...
function tm_updateTrialsPick() {
	...
	let trialsPickChampionButtonHolder = document.getElementById(
...
async function tm_trialsPickRoleHero(roleId, heroId, prismatic) {
	...
	let trialsChampionSelect = document.getElementById(`trialsChampionSelect`);
	let trialsCostSelect = document.getElementById(`trialsCostSelect`);
	let trialsPickRoleHeroButton = document.getElementById(
		`trialsPickRoleHeroButton`
	);
	let trialsStartTrialButton = document.getElementById(
		`trialsStartTrialButton`
	);
	let trialsChangeRoleHeroButton = document.getElementById(
		`trialsChangeRoleHeroButton`
	);
	let trialsLeaveLobbyButton = document.getElementById(
		`trialsLeaveLobbyButton`
	);
	...
	if (response.success & response.okay) {
		...
		txt += tm_addInfoRow(
			`- ${successType}:`,
			type == `pick`
				? `${champsById[heroId]} to assault the ${roles[roleId]}`
...
function tm_displayRunningTrial(wrapper, trialsInfo, campaign) {
	...
	let completed = false;
	if (
		campaign.dps_bonuses_earned != undefined &&
		campaign.dps_bonuses_earned[day - 1] != undefined &&
		day > 0
	)
...
function tm_parseRoles(trialsRoles) {
	roles = {};
	for (let role of trialsRoles) roles[role.id] = role.name.replace(/ .*/, "");
	return roles;
}
...
function tm_parseOwnedChampsByName(champsById, detailsHeroes) {
	...
	for (let ownedHero of detailsHeroes)
		if (ownedHero.owned == 1) ownedIds.push(Number(ownedHero.hero_id));
	let ownedByName = {};
